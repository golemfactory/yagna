...
mod tests {
    ...
    async fn test_counter_initial_proposal() -> Result<(), anyhow::Error> {
        ...
        let counter_proposal = proposal.counter_demand(example_demand())?;
        let new_proposal_id = market1
            .requestor_engine
            .counter_proposal(&subscription_id, proposal.proposal_id()?, &counter_proposal)
            .await?;
        assert_ne!(&new_proposal_id, proposal.proposal_id()?);
