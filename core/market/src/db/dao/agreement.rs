...
impl<'c> AgreementDao<'c> {
    ...
    pub async fn terminate(&self, id: &AgreementId, reason: Option<String>, owner_type: OwnerType) -> DbResult<bool> {
        let id = id.clone();
        do_with_transaction(self.pool, move |conn| terminate(conn, &id, reason, owner_type)).await
    }
...
fn terminate(conn: &ConnType, id: &AgreementId, reason: Option<String>, owner_type: OwnerType) -> DbResult<bool> {
    ...
    if num_updated == 0 { return Ok(false); }
