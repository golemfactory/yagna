    ...
    async fn resolve(&self) -> anyhow::Result<NodeId> {
        match self {
            ...
            NodeOrAlias::Alias(alias) => {
                let id = bus::service(identity::IDENTITY_SERVICE_ID)
    ...
    pub async fn run_command(&self, _ctx: &CliCtx) -> Result<CommandOutput> {
        match self {
            IdentityCommand::List { .. } => {
                let mut identities: Vec<identity::IdentityInfo> = bus::service(identity::IDENTITY_SERVICE_ID)
            ...
            IdentityCommand::Show { node_or_alias } => {
                ...
                CommandOutput::object(
                    bus::service(identity::IDENTITY_SERVICE_ID)
                        .send(command)
                        .await
                        .map_err(|e| anyhow::Error::msg(e))?,
                )
