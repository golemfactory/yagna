...
impl<'c> OrderDao<'c> {
    ...
    pub async fn get_orders(
        ...
        readonly_transaction(self.pool, "order_dao_get_orders", move |conn| {
            let orders = dsl::pay_order
                .left_join(
                    invoice_dsl::pay_invoice.on(dsl::invoice_id
                        .eq(invoice_dsl::id.nullable())
    ...
    pub async fn get_batch_items(
        ...
        readonly_transaction(self.pool, "order_dao_get_batch_items", move |conn| {
            let data: Vec<(String, String, String, bool)> = odsl::pay_batch_order
                ...
                .inner_join(oidsl::pay_batch_order_item
                    .on(
                        odsl::id.eq(oidsl::order_id)
                        .and(oidsl::owner_id.eq(oidsl::owner_id))
                    ))
                .select((
                    odsl::payer_addr,
                    oidsl::payee_addr,
