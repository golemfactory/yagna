...
async fn main() -> anyhow::Result<()> {
    ...
    ya_net::bind_remote(&net_host, &provider_id).await?;
    ...
    let provider_identity = Identity {
        identity: NodeId::from_str(provider_id).unwrap(),
        name: "".to_string(),
        role: "".to_string(),
    };
    ...
    HttpServer::new(move || {
        let payment_service = Scope::new(PAYMENT_API_PATH)
            .data(db.clone())
            .service(ya_payment::api::provider_scope());

        App::new()
            .wrap(middleware::Logger::default())
            .wrap(DummyAuth::new(provider_identity.clone()))
