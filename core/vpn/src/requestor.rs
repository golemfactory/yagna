...
/// Raw VPN connections are experimental and disabled unless
/// `YA_VPN_RAW_SOCKET_ENABLED` is set to `1` or `true`.
fn raw_socket_enabled() -> bool {
    std::env::var("YA_VPN_RAW_SOCKET_ENABLED")
...
async fn connect_raw(
    ...
    if !raw_socket_enabled() {
        return Err(ApiError::Vpn(VpnError::Forbidden));
    }
