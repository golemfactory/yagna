...
use crate::common::{
    authorize_agreement_initiator, generate_id, get_activity_agreement, get_agreement,
...
async fn create_activity(
    ...
    authorize_agreement_initiator(&identity_id, &agreement_id).await?;
    ...
    db.as_dao::<ActivityDao>()
        .create(&activity_id, &identity_id, &agreement_id)
        .await?;
...
async fn destroy_activity(
    ...
    let caller = format!("/net/{}", identity_id);
    let uri = provider_activity_service_id(&agreement)?;
    let _ = gsb_send!(caller, msg, &uri, query.timeout)?;
...
async fn exec(
    ...
    let caller = format!("/net/{}", identity_id);
    let uri = provider_activity_service_id(&agreement)?;
    gsb_send!(caller, msg, &uri, query.timeout)?;
...
async fn get_batch_results(
    ...
    let caller = format!("/net/{}", identity_id);
    let uri = provider_activity_service_id(&agreement)?;
    let results = gsb_send!(caller, msg, &uri, query.timeout)?;
