...
/// Yagna local network MAC address prefix (first 2 bytes).
/// Convention: MAC = [0xA0, 0x13, <4 bytes of IP address>]
const YAGNA_NET_MAC_PREFIX: [u8; 2] = [0xA0, 0x13];
        ...
        if ip_pkt.is_broadcast() {
            ...
            match networks.endpoint(ip) {
                ...
                None => {
                    ...
                    if dst_mac[0] == YAGNA_NET_MAC_PREFIX[0]
                        && dst_mac[1] == YAGNA_NET_MAC_PREFIX[1]
                    {
