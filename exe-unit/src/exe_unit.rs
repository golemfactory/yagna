...
use crate::service::{self, ServiceAddr, ServiceControl};
...
impl<R: Runtime> ExeUnit<R> {
    ...
    pub fn offer_template(binary: PathBuf, args: Vec<String>) -> crate::Result<OfferTemplate> {
    ...
    pub fn test(binary: PathBuf, args: Vec<String>) -> crate::Result<std::process::Output> {
        use crate::runtime::process::RuntimeProcess;
    ...
    pub(crate) async fn stop_runtime(runtime: Addr<R>, reason: ShutdownReason) {
...
#[derive(Clone)]
pub struct RuntimeRef<R: Runtime>(Addr<ExeUnit<R>>);

impl<R: Runtime> RuntimeRef<R> {
    pub fn from_ctx(ctx: &Context<ExeUnit<R>>) -> Self {
...
impl<R: Runtime> RuntimeRef<R> {
    ...
    async fn exec_stateless(&self, runtime_cmd: &ExecuteCommand) -> crate::Result<()> {
    ...
    async fn exec_stateful(
        ...
        let state_cur = self.send(crate::message::GetState {}).await?.0;
...
pub async fn report<S, M>(url: S, msg: M) -> bool
where
...
impl Handler<Shutdown> for TransferService {
    type Result = ResponseFuture<crate::Result<()>>;
