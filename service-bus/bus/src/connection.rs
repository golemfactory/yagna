    ...
    H: CallRequestHandler + 'static,
{
    fn handle(&mut self, item: Result<GsbMessage, ProtocolError>, ctx: &mut Self::Context) {
        ...
        match item.unwrap() {
            ...
            GsbMessage::Ping => {
                if let None = self.writer.write(GsbMessage::Pong) {
                    log::error!("error sending pong: Failed to write to sink");
                    ctx.stop();
                }
            }
            m => {
...
fn send_cmd_async<A: Actor, W: Sink<GsbMessage, Error = ProtocolError> + Unpin + 'static>(
    ...
    queue.push_back(tx);
    if let None = writer.write(msg) {
        ActorResponse::reply(Err(Error::GsbFailure(
            "Failed to write to sink".to_string(),
        )))
    } else {
