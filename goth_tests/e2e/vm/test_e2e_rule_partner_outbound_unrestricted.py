...
async def test_e2e_rule_partner_outbound_unrestricted(
    ...
    async with runner(goth_config.containers):
        ...
        manifest = open(
            f"{runner.web_root_path}/outbound_manifest_unrestricted.json"
        ).read()
        node_descriptor = json.loads(
            open(
                f"{runner.web_root_path}/test_e2e_rule_partner_outbound/node-descriptor-unrestricted.signed.json"
            ).read()
        )
